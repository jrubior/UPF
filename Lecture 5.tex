...
\begin{itemize}
    ...
    \item $\beta_k$ is the expected difference in $Y_i$ associated with a unit change in $X_{ki}$ holding all other $X_{kj}_{\{j\neq i\}}$ constant
    \item $\beta_0$, the intercept, can be thought of as the coefficient on a regressor $X_{0i}$ that is equal to 1 for all observations. $\beta_0$ is the expected value of $Y_i$ when all $X_ki$ are equal to zero.
\end{itemize}
...
\end{equation*}
In general, holding all other $X_{kj}_{\{j\neq i\}}$ constant, $\frac{\Delta Y_i}{\Delta X_{ki}}= \beta_k$. 
\end{frame}
