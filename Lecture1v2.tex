...
\title[]{\textcolor{blue}{Lecture 1 v2:\\Introduction and Review of Probability}}
...
\begin{itemize}
    \item[--] \textit{\color{teal}{Participation:}} 5\% + \textit{\color{orange}{Midterm 1:}} 15\% + \textit{\color{orange}{Midterm 2:}} 20\% + \textit{\color{purple}{Cumulative Final Exam:}} 60\%
\end{itemize}
...
\begin{frame}[label=intro_outline]{Course Outline}
\footnotesize
\begin{columns}[T]
    \begin{column}{.5\textwidth}
        ...
        \setlength{\itemsep}{0pt}
            ...
            \item[$\longrightarrow$] {\color{orange}{Midterm 1}}
        \end{enumerate}
        ...
        \setcounter{enumi}{4}
            ...
            \item[$\longrightarrow$] {\color{orange}{Midterm 2}}
        \end{enumerate}
    \end{column}
    \begin{column}{.5\textwidth}
        \begin{center}
        ...
        \setcounter{enumi}{7}
            ...
            \item[$\longrightarrow$] {\color{purple}{Cumulative Final Exam}}
        \end{enumerate}
    \end{column}
\end{columns}
\end{frame}
...
\begin{equation*}
    ...
    &\equiv \sum_i \mathbb{E}(Y\mid X=x_i) Pr(X=x_i)
    \end{aligned}
\end{equation*}
...
\end{eqnarray*} 
 \begin{itemize}
    ...
    \item The converse is not true in general: zero covariance does not necessarily imply independence.\vspace{.2cm}
...
\begin{frame}{Random Sampling}
    \begin{itemize}
        ...
        \item When $y_1, y_2, ..., y_n$ are randomly drawn from the same distribution, they are independently and identically distributed (i.i.d.). Then, the expected value of every draw is the population mean and the variance of every draw is the population variance.
    \end{itemize}
\end{frame}
...
\begin{frame}{Asymptotic properties ($n\rightarrow\infty$) of the sampling distribution (2/2)}
    \begin{itemize}
        ...
        \item If $y_1, y_2, ..., y_n$ are drawn randomly from the same population (i.i.d.) and large outliers are unlikely, then $\overline{Y}$ is approximately distributed as $\mathcal{N}(\mu_Y, \sigma^2_Y/n)$ as $n$ grows
        \begin{itemize}
    ...
    \begin{exampleblock}{Central Limit Theorem}
        ...
        \begin{equation*}
            \frac{\overline{Y}-\mu_Y}{\sigma_Y/\sqrt{n}} \xrightarrow{d} \mathcal{N}(0, 1)
        \end{equation*}
    \end{exampleblock}
\end{frame}
...
\begin{itemize}
    ...
    \begin{enumerate}
        ...
        \item $\hat{\mu}_y$ is \textit{more efficient}  than $\tilde{\mu}_y$ if $\operatorname{var}(\hat{\mu}_y) \leq \operatorname{var}(\tilde{\mu}_y)$
    \end{enumerate}
\end{itemize}
\end{frame}
...
\begin{itemize}
    ...
    \begin{enumerate}
        ...
        & var(\hat{\mu}_y)\geq \mathbb{I}_{\mu_Y}^{-1} = \frac{\sigma_Y^2}{n} &\text{\small (Cramer-Rao Inequality + Central Limit Theorem)}\\
