...
\end{equation*}
\begin{equation*}
\Rightarrow \beta_1 = \frac{cov(Z_i, Y_i)}{cov(Z_i, X_i)}
\end{equation*}
The IV estimator (replacing population covariances with the sample covariances) is a consistent estimator of $\beta_1$:
\begin{equation*}
\hat{\beta}_1^{2SLS}= \frac{s_{ZY}}{s_{ZX}} \overset{p}{\to} \frac{cov(Z_i, Y_i)}{cov(Z_i, X_i)} = \beta_1
\end{equation*}
\begin{center}
Note that $\hat{\beta}_1^{OLS}= \frac{s_{XY}}{s_{XX}}$.
\end{center}
...
\begin{frame}{Summary}
\begin{itemize}
    \item A valid instrument lets us isolate a part of $X$ that is uncorrelated with $u$, and that part can be used to estimate the effect of a change in $X$ on $Y$
    \vspace{1em}
    ...
    \end{enumerate}
    \vspace{1em}
    \item A valid instrument isolates variation in $X$ that is ``as if'' randomly assigned.
    \vspace{1em}
