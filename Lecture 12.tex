...
\begin{frame}{Nonlinear Models}
So far, we have treated $Pr(y_i=1 \mid x_i')$ linearly. Now, assume that this probability is given by a distribution function $F(x_i'\beta)$. \\~\\
...
\begin{itemize}
    \item Likelihood Ratio Test:
    \[
    LR=2[\mathscr{l}(\hat{\beta}_N)-\mathscr{l}(\hat{\beta}_R)] \sim \chi_q^2
    \]
Where N and R represent the unrestricted and restricted (i.e., without explanatory variables) models respectively, and q is the difference in parameters. If we reject $H_0$, the unrestricted model will be better. \\~\\

\item Number of correct predictions: we assign $\hat{y}_i=1$ if $\hat{P}_i>a$ and count the number of incorrect predictions from the model $\sum_{i=1}^N (y_i-\hat{y}_i)^2$\\~\\
...
\item LPM:
\[
E[infl_i \mid x_i]=x_i'\beta \ \ {\color{MCO}}
\]
\item  Probit:
\[
E[infl_i \mid x_i]=\Phi(x_i'\beta) \ \ {\color{MV}}
\]
\item Logit:
\[
E[infl_i \mid x_i]=\frac{e^{x_i'\beta}}{1+e^{x_i'\beta}} \ \ {\color{MV}}
\]
\end{itemize}
\end{frame}
